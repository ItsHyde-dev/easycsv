...
pub fn print_duplicates(mut reader: Reader<Box<dyn Read>>, dup_rows: Vec<String>) {
    let pos_list = select::get_selected_header_position_list(&mut reader, dup_rows.clone());
...
pub fn print_duplicates_v2(
    i: Box<dyn Iterator<Item = StringRecord>>,
    dup_rows: Vec<String>,
    headers: &Vec<String>,
) {
    let pos_list = select::get_selected_header_position_list_v2(headers, dup_rows.clone());
    let pos_map = get_selected_header_position_map_v2(headers, dup_rows);
...
pub fn get_selected_header_position_map_v2(
    headers: &Vec<String>,
    select: Vec<String>,
) -> HashMap<usize, String> {
    let mut pos_list: HashMap<usize, String> = HashMap::new();
